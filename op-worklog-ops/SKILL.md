---
name: op-worklog-ops
description: OpenProject munkanaplo muveletek op CLI-val: bejegyzes rogzitese, napi riport, szoveges kereses, es automatikus napi 8 oras kiegeszito logolas.
---
...
5. `Uj work package letrehozas`  
   Uj work package letrehozasa es automatikus hozzarendeles az aktualis felhasznalohoz.
...
### Uj work package letrehozasa

Parancs minta:

```bash
op create "Cim" ["Leiras"] [--projectId=<id>]
```

Szabalyok:
- Uj work package-et hoz letre, es automatikusan hozzad rendeli.
- Projektvalasztas sorrendje:
  - `--projectId=<id>` (vagy `--projectId <id>`) az elsoseges;
  - ha ez nincs megadva, az `op init` soran beallitott `.op_info` projektet hasznalja.
- A CLI lekerozi a felhasznalo ID-t: `/api/v3/users/me`.
- Ha leiras is erkezik, Markdown formatumban kuldi.
- Sikeres valasz eseten kompakt JSON-t ir ki: `id`, `title`, `status`.
...
### 2) Uj work package letrehozas

Pelda:

```bash
op create "API hibajavitas" "Hibakezeles pontositasa a create endpointben" --projectId=42
```
...
### 5) Automatikus napi kiegeszites (ma)
